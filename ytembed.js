...
var ytEmbed = {
    ...
    init: function(cfg) {
        ...
        if (!this.cfg.key) {
                this.cfg.key = 'AIzaSyA5ZZvBKfiSALNlrV0iArNpSmEPRoGJBwI';
        }
        ...
        if (json.error) {
            ...
            for (var i = 0; i < json.items.length; i++) {
                ...
                if (this.cfg.layout == 'thumbnails') {
                    ...
                    //this.cfg.layout = full
                    li.innerHTML = '<table class="table"><tr><td valign="top" rowspan="2"></td><td valign="top"><h3>' + entry.snippet.title + '</h3><span>' + this.formatDescription(entry.snippet.description) + '</span></td><td valign="top" style="width: 150px" class="meta"><div>' + (entry.contentDetails ? 'Duration: ' + ytEmbed.formatDuration(entry.contentDetails.duration) + '<br>' : '') + (entry.statistics ? 'Views: ' + entry.statistics.viewCount + '<br>' : '') + 'From: <a href="http://www.youtube.com/profile?user=' + entry.snippet.channelTitle + '">' + entry.snippet.channelTitle + '</a></div></td></tr></table>';
